...
data<-final_data %>% select(contains('14'))
zip<-final_data[,1]
...
cross_final<-mutate(cross_final, 
                    ...
                    below_pov_14 = 100*(below_pov_14/poo_14)
                    )
###################################Fastfood#############################################################
###################################Fastfood#############################################################
###################################Fastfood#############################################################
m.fastfood_g<- glm.nb(fastfood14 ~ per_black +per_asian + per_white + per_hispanic
                    + below_pov_14/pop_14 + factor(msa), data = cross_final, offset(pop_14))
m.fastfood_bc<- glm.nb(fastfood14 ~ per_black +per_asian + per_other + per_hispanic
                    + below_pov_14/pop_14  + factor(msa), data = cross_final, offset(pop_14))
summary(m.fastfood_g)
summary(m.fastfood_bc)
summary(m.fastfood_g)
exp(m.fastfood_bc$coefficients)
class(m.fastfood)

bc_fastfood_coef<-data.frame(exp(m.fastfood_bc$coefficients)[2:5])
bc_fastfood_coef$race<-c('% Black', '% Asian', '% Other', '% Hispanic')
...
bc_fastfood<- plot_ly(bc_fastfood_coef, x =~ race, y =~ coef, type = 'bar', text =~ text, marker = list(color =c('red', 'orange', 'green', 'black') ))  %>% layout(
  shapes=list(type='line', x0= -1, x1= 5, y0=1, y1=~1, line=list(dash='dot', width=1)),
  title = 'Negative Binomial Regression Results',
  yaxis = list(title = 'exponentiated coefficients', range = c(.97,1.05)),
...
summary(m.freshfood_g)
exp(m.freshfood_bc$coefficients)
class(m.freshfood)

bc_freshfood_coef<-data.frame(exp(m.freshfood_bc$coefficients)[2:5])
bc_freshfood_coef$race<-c('% Black', '% Asian', '% Other', '% Hispanic')
...
bc_freshfood<- plot_ly(bc_freshfood_coef, x =~ race, y =~ coef, type = 'bar', text =~ text, marker = list(color =c('red', 'orange', 'green', 'black') ))  %>% layout(
  shapes=list(type='line', x0= -1, x1= 5, y0=1, y1=~1, line=list(dash='dot', width=1)),
  title = 'Negative Binomial Regression Results',
  yaxis = list(title = 'exponentiated coefficients', range = c(.97,1.05)),
  xaxis = list(title = '% Race (white as the reference)')) 
...
summary(m.unfreshgroceries_g)
exp(m.unfreshgroceries_bc$coefficients)


bc_unfreshgroceries_coef<-data.frame(exp(m.unfreshgroceries_bc$coefficients)[2:5])
bc_unfreshgroceries_coef$race<-c('% Black', '% Asian', '% Other', '% Hispanic')
...
bc_unfreshgroceries<- plot_ly(bc_unfreshgroceries_coef, x =~ race, y =~ coef, type = 'bar', text =~ text, marker = list(color =c('red', 'orange', 'green', 'black') ))  %>% layout(
  shapes=list(type='line', x0= -1, x1= 5, y0=1, y1=~1, line=list(dash='dot', width=1)),
  title = 'Negative Binomial Regression Results',
  yaxis = list(title = 'exponentiated coefficients', range = c(.97,1.05)),
  xaxis = list(title = '% Race (white as the reference)')) 
...
summary(m.alcohol_g)
exp(m.alcohol_bc$coefficients)
class(m.alcohol)

bc_alcohol_coef<-data.frame(exp(m.alcohol_bc$coefficients)[2:5])
bc_alcohol_coef$race<-c('% Black', '% Asian', '% Other', '% Hispanic')
...
bc_alcohol<- plot_ly(bc_alcohol_coef, x =~ race, y =~ coef, type = 'bar', text =~ text, marker = list(color =c('red', 'orange', 'green', 'black') ))  %>% layout(
  shapes=list(type='line', x0= -1, x1= 5, y0=1, y1=~1, line=list(dash='dot', width=1)),
  title = 'Negative Binomial Regression Results',
  yaxis = list(title = 'exponentiated coefficients', range = c(.97,1.05)),
  xaxis = list(title = '% Race (white as the reference)')) 
